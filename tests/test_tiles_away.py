"""
Tiles-away tests using known hand configurations.
...
from cracked.tiles_away import (
    tiles_away, tiles_away_standard, tiles_away_seven_pairs,
    tiles_away_thirteen_orphans, acceptance_count, best_discards,
)
...
def test_complete_seven_pairs():
    ...
    assert tiles_away(h) == -1
    assert tiles_away_seven_pairs(h) == -1

def test_complete_thirteen_orphans():
    h = hand("b1","b9", "c1","c9", "d1","d9", "ew","sw","ww","nw", "rd","gd","wd","b1")
    assert tiles_away(h) == -1
    assert tiles_away_thirteen_orphans(h) == -1


# ---------------------------------------------------------------------------
# Waiting hands (tiles_away = 0)
# ---------------------------------------------------------------------------

def test_waiting_single_wait():
    # Waiting on b3 to complete b1-b2-b3
    h = hand("b1","b2", "c1","c2","c3", "d1","d2","d3", "ew","ew","ew", "rd","rd")
    assert tiles_away(h) == 0

def test_waiting_seven_pairs():
    # 6 pairs + 1 single = waiting for 7th pair
    h = hand("b1","b1", "b2","b2", "b3","b3", "c1","c1", "c2","c2", "c3","c3", "d1")
    assert tiles_away_seven_pairs(h) == 0
    assert tiles_away(h) == 0

def test_waiting_thirteen_orphans_no_pair():
    # All 13 orphan types, no pair
    h = hand("b1","b9","c1","c9","d1","d9","ew","sw","ww","nw","rd","gd","wd")
    assert tiles_away_thirteen_orphans(h) == 0
    assert tiles_away(h) == 0

def test_waiting_thirteen_orphans_with_pair():
    # 12 unique orphans + 1 pair among them = waiting (waiting on the missing orphan)
    h = hand("b1","b1","b9","c1","c9","d1","d9","ew","sw","ww","nw","rd","gd")
    assert tiles_away_thirteen_orphans(h) == 0


# ---------------------------------------------------------------------------
# Various tiles_away distances
# ---------------------------------------------------------------------------

def test_tiles_away_1():
    # One tile away from waiting
    h = hand("b1","b2", "b4","b5", "c1","c2","c3", "d1","d2","d3", "ew","ew","ew")
    assert tiles_away(h) == 1

def test_tiles_away_2():
    # Relatively scattered hand
    h = hand("b1","b3","b5","b7","b9","c1","c3","c5","c7","c9","d1","d3","d5")
    s = tiles_away(h)
    assert s >= 2

def test_tiles_away_with_exposed_meld():
    # 1 exposed pong reduces groups_needed to 3
    h = hand("b1","b2", "c1","c2","c3", "d1","d2","d3", "ew","ew")  # 10 concealed
    s = tiles_away_standard(h, exposed_melds=1)
    assert s == 0  # waiting waiting on b3

def test_all_isolated_tiles_tiles_away():
...
def test_acceptance_waiting_hand():
    # Waiting on b3 or b6 (two-sided wait on b4-b5)
...
def test_acceptance_empty_for_complete_hand():
    # Complete hand has tiles_away -1; no tile reduces it further
    h = hand("b1","b2","b3", "c4","c5","c6", "d7","d8","d9", "ew","ew","ew", "rd","rd")
    wall = full_wall()
    acc = acceptance_count(h, wall)
    assert len(acc) == 0
...
def test_best_discards_prefers_lower_tiles_away():
    # 14-tile hand: 4 complete groups + 2 isolated honors (rd, gd)
    # Discarding rd or gd leaves 4 groups + 1 isolated tile = waiting (tiles_away=0)
    # Discarding any suited tile breaks a group and gives tiles_away=1
